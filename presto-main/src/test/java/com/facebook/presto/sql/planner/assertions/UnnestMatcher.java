        ...
        implements Matcher
{
    ...
    public MatchResult detailMatches(PlanNode node, StatsProvider stats, Session session, Metadata metadata, SymbolAliases symbolAliases)
    {
        ...
        if (alias.size() != unnestNode.getOutputVariables().size()) {
            return MatchResult.NO_MATCH;
        }

        SymbolAliases.Builder builder = SymbolAliases.builder().putAll(symbolAliases);
        IntStream.range(0, alias.size()).forEach(i -> builder.put(alias.get(i), new SymbolReference(unnestNode.getOutputVariables().get(i).getName())));
